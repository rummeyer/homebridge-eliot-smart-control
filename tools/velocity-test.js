#!/usr/bin/env node
/**
 * Does VELOCITY (0x13) change anything? THIS MOVES THE DESK.
 *
...
for (const value of [40, 28, 40]) {
  ...
  await link.send(Cmd.VELOCITY, [value]);
  await sleep(1200);
  const speed = await driveTo(HIGH);
  say(`VELOCITY ${value}: ${LOW}→${HIGH} mm, marschgeschwindigkeit ${speed ? speed.toFixed(1) : '?'} mm/s`);
}
